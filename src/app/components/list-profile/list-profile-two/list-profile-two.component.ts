...
export class ListProfileTwoComponent implements OnInit {
  ...
  listProfile(categoryName: any, subCategoryName: any, subcategory2Name: any) {
    if (
      ...
      this.profiles = [];
      this.realtime
        .listProfileByCategoryEmployer(categoryName)
        .snapshotChanges()
        .subscribe((data) => {
          data.forEach((k) => {
            ...
            this.profiles = this.profiles.filter(
              (el) => el.type == 'Employer' && el.rol !== 'admin'
            );
          });
        });
    } else if (
      ...
      this.profiles = [];
      this.realtime
        .listProfileByCategoryEmployer(subCategoryName)
        .snapshotChanges()
        .subscribe((data) => {
          data.forEach((k) => {
            const x = { ...k.payload.toJSON(), key: k.key };
      ...
      this.profiles = [];
      this.realtime
        .listProfileByCategoryEmployer(subcategory2Name)
        .snapshotChanges()
        .subscribe((data) => {
          data.forEach((k) => {
            const x = { ...k.payload.toJSON(), key: k.key };
