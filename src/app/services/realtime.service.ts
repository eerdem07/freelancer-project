...
export class RealtimeService {
  ...
  simdikiKullaniciByKey() {
    return this.db.list('/Users', (q) =>
      q.orderByChild('userID').equalTo(this.suankiKullanici.uid)
    );
  }
  ...
  listProfileByUserID(userID: string) {
    return this.db.list<User>('/Users', (q) =>
      q.orderByChild('userID').equalTo(userID)
    );
  }
  ...
  listJobCategoryByName(name: string) {
    return this.db.list('JobCategory', (q) =>
      q.orderByChild('name').equalTo(name)
    );
  }
  ...
  async addSubCategory2(subCategory2: subCategory2, key1: any, key2: any) {
    await this.db
      .list<subCategory2>('/Category/' + key1 + '/' + key2)
      .push(subCategory2);
  }
  ...
  listCategoryByName(name: string) {
    return this.db.list('/Category', (q) =>
      q.orderByChild('name').equalTo(name)
    );
  }

  listSubCategoryByName(name: string, key: any) {
    return this.db.list('/Category/' + key, (q) =>
      q.orderByChild('name').equalTo(name)
    );
  }
  ...
  listProfileByCategoryPersonel(categoryName: any) {
    return this.db.list('/Users', (q) =>
      q.orderByChild('bolum').equalTo(categoryName)
    );
  }

  listProfileBySubCategoryPersonel(categoryName: any) {
    return this.db.list('/Users', (q) =>
      q.orderByChild('altKategori').equalTo(categoryName)
    );
  }

  listProfileBySubCategory2Personel(categoryName: any) {
    return this.db.list('/Users', (q) =>
      q.orderByChild('yetenekler').equalTo(categoryName)
    );
  }
  ...
  listProfile() {
    return this.db.list<User>('/Users', (q) =>
      q.orderByChild('userID').limitToFirst(10)
    );
  }
}
